...
def joystick_callback(joy):
    ...
    r2, l2 = joy.axes[R2], joy.axes[L2]

    if depth_lock == 1:
        if time.time() - current_time >= period:
            if r2 < 1 and l2 == 1:
                depth_setpoint = int(depth_setpoint + funcs.map_val(r2, 1, -1, 0, 10))
            elif r2 == 1 and l2 < 1:
                depth_setpoint = int(depth_setpoint - funcs.map_val(l2, 1, -1, 0, 10))
            
            depth_setpoint_pub.publish(depth_setpoint)
